...
def apply_medical_augmentation(image: Image.Image) -> Tuple[Image.Image, AugParams]:
    ...
    for tech in selected:
        if tech == "rotate":
            ...
            # scale to counts approx (lambda), add poisson noise
            # use clip to avoid extreme quantization on near-zero images
            ...
            if cv2 is None:
                logger.warning("cv2 not available; falling back to ImageOps.equalize for CLAHE step.")
                augmented = ImageOps.equalize(augmented)
            else:
                # ensure uint8
                if arr.dtype != np.uint8:
                    arr = (np.clip(arr, 0.0, 1.0) * 255.0).astype(np.uint8) if arr.max() <= 1.0 else arr.astype(np.uint8)
                # single-channel
                cl = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=tile_grid).apply(arr)
                augmented = Image.fromarray(cl)

        elif tech == "elastic":
            # Conservative elastic deformation (only if scipy.ndimage present)
            if ndi is None:
                logger.warning("scipy.ndimage not available; skipping elastic augmentation.")
            ...
            warped = ndi.map_coordinates(arr, indices, order=1, mode="reflect").reshape(h, w)
...
def copy_files_between_splits(source_root: Path, split_from: str, split_to: str, classes: List[str], random_seed: int = 42) -> int:
...
def setup_dataset(original_path: Path, target_path: Path, augment_flag: int, seed: Optional[int] = None) -> bool:
    _set_seed(seed)
    ...
    for split in ["train", "test"]:
        for class_name in classes:
            ...
            logger.info(f"Copying {'and augmenting ' if split == 'train' and augment_flag == 1 else ''}{len(files)} files from {src} to {dst}")
                    ...
                    try:
                        with Image.open(p) as im:
                            ...
                            for i in range(aug_count):
                                try:
                                    aug_img, aug_params = apply_medical_augmentation(img_gray)
                                    name = p.stem
                                    ext = p.suffix or ".png"
                                    ...
                                    logger.error(f"Augmentation iteration failed for {p.name}: {e}")
                    except Exception as e:
...
def parse_args():
    ...
    parser.add_argument("--augment", type=int, choices=[0, 1], required=True, help="0 = no augmentation, 1 = apply augmentation")
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducibility (default: None)")
    parser.add_argument("--force", action="store_true", help="If provided, will remove existing target dataset directory")
    parser.add_argument("--original", type=str, default=r"E:\medical_image_classification\labeled-chest-xray-images", help="Original dataset root")
    parser.add_argument("--target", type=str, default=r"E:\medical_image_classification\splited_chest_xray_2", help="Target dataset root")
...
def main():
    ...
    if target_dataset.exists():
        if args.force:
            ...
            logger.error(f"Target dataset already exists: {target_dataset}. Use --force to remove.")
            return

    success = setup_dataset(original_dataset, target_dataset, args.augment, seed=args.seed)
    if success:
