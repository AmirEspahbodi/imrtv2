...
class CoAtNetSideViTClassifier_1(nn.Module):
    ...
    def forward(self, x: torch.Tensor, K_value=None, Q_value=None) -> torch.Tensor:
        ...
        vit_out1 = self.sidevit1(
            F.interpolate(
                sv1_in, size=(128, 128), mode="bilinear", align_corners=False
            ),
            K_value,
            Q_value,
        )
        vit_out2 = self.sidevit2(
            F.interpolate(
                sv2_in, size=(128, 128), mode="bilinear", align_corners=False
            ),
            K_value,
            Q_value,
        )
...
class CoAtNetSideViTClassifier_4(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, key_states, value_states
    ) -> torch.Tensor:  # Accept extra args to match user's call
        x_backbone = F.interpolate(
            x, size=(224, 224), mode="bilinear", align_corners=False
        )
        ...
        vit_input1 = F.interpolate(
            vit_input1, size=(128, 128), mode="bilinear", align_corners=False
        )
        vit_input2 = F.interpolate(
            vit_input2, size=(128, 128), mode="bilinear", align_corners=False
        )
