...
class FusionModule(torch.nn.Module):
    def __init__(
        ...
        assert prompt_proj or self.prompt_dim == in_dim, (
            "Prompt projection is required when prompt dimension is different from input dimension."
        )

        self.prompts = nn.Parameter(
            torch.zeros(num_layers, 1, num_prompts, self.prompt_dim)
        )
        self.layer_norm = (
            nn.LayerNorm(self.prompt_dim) if prompt_norm else nn.Identity()
        )
        self.prompt_proj = (
            nn.Linear(self.prompt_dim, in_dim) if prompt_proj else nn.Identity()
        )
    ...
    def forward(self, key_layer, value_layer):
        assert key_layer.shape[0] == self.num_layers, (
            "Key layer should have the same number of layers as the fusion module."
        )
        assert value_layer.shape[0] == self.num_layers, (
            "Value layer should have the same number of layers as the fusion module."
        )
